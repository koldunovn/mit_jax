...
| production config on one A100 (FMA, cg2d unroll 5) vs Fortran 13-rank serial, 1992 (8760 steps) | daily global %MON: <= 1.1e-10 rel (most 1e-13); **full year (365 days): theta <= 1.2e-10, salt <= 6.5e-12, eta <= 6.2e-11, u/v <= 3.3e-9 rel** (compare_vs_fortran_serial_year.txt) |
...
| **full V4r4 (EXF bulk + sea ice), one month (744 steps) on CPU, gate flags** | **final state BITWISE** vs Fortran 13 ranks (serial13 and mpi13): u, v, theta, salt, etaN, AREA, HEFF, HSNOW, UICE, VICE all 0 differing values; every hourly %MON (ocean, sea ice, EXF) to print precision |
| **full V4r4, one year 1992 (8760 steps) on one GH200** (production flags, Pallas LSR; 0.29 s/step, 48 min) vs Fortran 13 ranks | every one of 124 daily %MON statistics closer to the Fortran 13-rank year than the Fortran 96-rank year is (typically 100-1000x): theta mean 6.6e-10 vs 4.2e-7, ice area mean 3.5e-8 vs 1.7e-5, ice thickness mean 1.5e-8 vs 1.3e-5 |
| **full V4r4 (EXF bulk + sea ice), one month on one GH200** (production XLA flags, Pallas LSR; 0.27 s/step) vs Fortran 13 ranks | every one of 124 hourly %MON statistics (ocean, sea ice, EXF) closer to the Fortran 13-rank run than the Fortran 96-rank run is (typically 10-1000x): e.g. mean theta 2e-10 vs 1.7e-8, SSH sd 2e-8 vs 2e-5, ice area mean 8.7e-7 vs 4.7e-6 |
