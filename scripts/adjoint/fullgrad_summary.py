...
def grads_by(rs):
    """{(days, mode, nproc): {repeat: row}}"""
    g = defaultdict(dict)
    for r in rs:
        if r["action"] == "grad":
            g[(r["days"], r["mode"], r.get("nproc", 1))][r.get("repeat", 0)] = r
    return g
...
def fd_tables(rs, ref_mode, P):
    ...
    if not fd:
        return
    P("### FD h-sweep vs adjoint, per named direction\n")
    P(f"Relative error |FD - AD| / |AD| at each h, AD = the {ref_mode} adjoint (r0); plateau = number of h with "
      f"rel. error <= {BAR_FD:g} (bar: >= 2 of 4); the other modes' adjoints relative to the best FD value.\n")
    P(f"| window | direction | AD {ref_mode} | TL {ref_mode} | FD rel. error at h = ... | plateau | other modes rel. "
      f"to best FD | FD job |")
    P("|---|---|---|---|---|---|---|---|")
    for days in sorted({r["days"] for r in fd}):
        modes = sorted({m for (d, m, n) in G if d == days and n == 1}, key=mode_sort)
        for n in sorted({r["direction"] for r in fd if r["days"] == days}):
            ...
            gref = G.get((days, ref_mode, 1), {}).get(0)
            if gref is None:
                continue
            ad = gref["dirderiv"][n]
            ...
            P(f"| {days:g} d | {n} | {ad:.6e} | {e(t['tl'], '{:.6e}') if t else '-'} | "
              ...
              + f" | {npl}/{len(sel)} | {'; '.join(others) or '-'} | {sel[0]['job']} |")
    P("")
...
def grad_tables(rs, P):
    ...
    P("|---|---|---|---|---|---|---|---|---|---|---|---|---|---|")
    for key in sorted(G, key=lambda k: (k[0], mode_sort(k[1]), k[2])):
        days, mode, nproc = key
        for rep, r in sorted(G[key].items()):
            ...
            P(f"| {days:g} d | {mode} | {nproc} | {rep} | {r['J']:.16g} | {mrs} | {r['forward_s']:.0f} | "
    ...
    for days in sorted({k[0] for k in G}):
        modes = sorted({k[1] for k in G if k[0] == days and k[2] == 1}, key=mode_sort)
        if len(modes) < 2:
            continue
        ref = G[(days, modes[0], 1)][0]["dirderiv"]
        P(f"| {days:g} d direction | " + " | ".join(modes) + " |")
        P("|---|" + "---|" * len(modes))
        for n in ref:
            vals = []
            for m in modes:
                v = G[(days, m, 1)][0]["dirderiv"][n]
